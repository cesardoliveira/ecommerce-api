...
RSpec.describe "Admin::V1::SystemRequirements as :admin", type: :request do
  ...
  context "GET /system_requirements" do
    ...
    it "returns all SystemRequirements" do
      ...
      expect(body_json['system_requirements']).to contain_exactly *system_requirements
      .as_json(only: %i(id name operational_system storage processor memory video_board))
    end
  ...
  context "POST /system_requirements" do
    ...
    context "with valid params" do
      ...
      it 'returns last added SystemRequirement' do 
        post url, headers: auth_header(user), params: system_requirement_params
        expected_system_requirement = SystemRequirement.last
        .as_json(only: %i(id name operational_system storage processor memory video_board))
        expect(body_json['expected_system_requirement']) == expected_system_requirement
      end
  ...
  context "PATCH /system_requirements/:id" do
    ...
    context "with valid params" do
      let(:new_name) { 'My updated System Requirements' }
      ...
      it 'updates SystemRequirement' do
        ...
        expect(system_requirement.name) == new_name
      end

      it 'returns updated system_requirement' do
        patch url, headers: auth_header(user), params: system_requirement_params
        system_requirement.reload
        expected_system_requirement = system_requirement
        .as_json(only: %i(id name operational_system storage processor memory video_board))
        expect(body_json['system_requirement']) == expected_system_requirement
      end
    ...
    context "with invalid params" do
      ...
      it 'does not update SystemRequirement' do
        ...
        system_requirement.reload
        expect(system_requirement.name) == old_name
      end
